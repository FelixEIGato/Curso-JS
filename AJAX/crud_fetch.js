...
const getAll = async () =>{
    try {
        let res = await fetch("http://localhost:3000/gatos"),
         json = await res.json();
        ...
        if(!e.target.id.value){
            try {
                let options = {
                 ...
                 res = await fetch("http://localhost:3000/gatos",options),
                 json = await res.json();
            ...
            try {
                let options = {
                 ...
                 res = await fetch(`http://localhost:3000/gatos/${e.target.id.value}`,options),
                 json = await res.json();
        ...
        if(isDelete){
            try {
                let options = {
                 ...
                 res = await fetch(`http://localhost:3000/gatos/${e.target.dataset.id}`,options),
                 json = await res.json();
